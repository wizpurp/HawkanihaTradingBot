...
def enrich_trade_rows(rows):
    ...
    for row in rows:
        ...
        if action == "BUY":
            ...
            buy_row = matching_buys.pop() if matching_buys else None
...
def summarize_ledger(rows, source):
    starting_balance = 500.0
    source_rows = [row for row in rows if trade_source(row) == source]
    sell_rows = [row for row in source_rows if row.get("Action") == "SELL"]
...
def get_trade_performance():
    ...
    return {
        "BOT": summarize_ledger(rows, "BOT"),
        "HUMAN": summarize_ledger(rows, "HUMAN")
    }
...
PnL: <span class="{pnl_class}">{pnl_display}</span><br>
Hold Time: {escape_html(trade.get("HoldTime") or "N/A")}<br>
Grade: {escape_html(trade.get("Grade") or "N/A")}<br>
Reason: {escape_html(trade.get("GradeReason") or "N/A")}<br>
...
def render_trade_list(summary):
    ...
    for trade in summary["trade_list"]:
        ...
        lines.append(
            ...
            f"""<span class="{pnl_class}">{fmt_money(trade["pnl"])}</span> """
...
Starting {escape_html(label.title())} Balance: {fmt_money(summary["starting_balance"])}<br>
Current {escape_html(label.title())} Balance: {fmt_money(summary["current_balance"])}<br>
