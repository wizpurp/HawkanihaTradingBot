...
TRADE_COLUMNS = [
    ...
    "TradeScore", "GradeReason", "HoldTime"
]
BOT_AUDIT_COLUMNS = [
...
def summarize_trades(rows):
    ...
    for row in sell_rows:
        try:
            ...
            pass
        grade_values.append(grade_value(row.get("BotGrade") or row.get("ExitGrade")))

    wins = [pnl for pnl in pnl_values if pnl > 0]
    best = max(pnl_values) if pnl_values else 0
    worst = min(pnl_values) if pnl_values else 0
    avg_grade_score = average(grade_values)
    hold_seconds = []
    for row in sell_rows:
        hold_time = row.get("HoldTime", "")
        try:
            if "day" in hold_time:
                day_part, time_part = hold_time.split(",", 1)
                days = int(day_part.split()[0])
            else:
                days = 0
                time_part = hold_time
            hours, minutes, seconds = [int(part) for part in time_part.strip().split(":")]
            hold_seconds.append(days * 86400 + hours * 3600 + minutes * 60 + seconds)
        except:
            pass

    avg_hold_seconds = int(average(hold_seconds)) if hold_seconds else 0
    avg_hold_time = str(timedelta(seconds=avg_hold_seconds)) if avg_hold_seconds else "N/A"

    return {
        "total_trades": len(sell_rows),
        "win_rate": (len(wins) / len(sell_rows) * 100) if sell_rows else 0,
        "average_grade": grade_from_score(avg_grade_score) if grade_values else "N/A",
        "average_pl": average(pnl_values),
        "best_trade": best,
        "worst_trade": worst,
...
def get_trade_analysis():
    ...
    bot_rows = [row for row in rows if (row.get("Source") or "HUMAN") == "BOT"]
    human_rows = [row for row in rows if (row.get("Source") or "HUMAN") != "BOT"]

    return {
        "all": summarize_trades(rows),
        "bot": summarize_trades(bot_rows),
        "human": summarize_trades(human_rows)
    }
...
def dashboard():
    ...
    last_review = bot_snapshot["last_trade_review"]
    levels = market_context.get("levels", {})
...
Confidence: <span id="bot-confidence">{bot_snapshot["confidence"]}</span><br>
...
Entry Grade: {pl.get("entry_grade") or "N/A"}<br>
Live Grade: {pl.get("live_grade") or "N/A"}<br>
Current Grade Reason:<br>
{pl.get("live_reason") or "N/A"}<br>
Status: OPEN
</div>
<br>
"""
...
Entry Grade: {trade.get("EntryGrade", "")}<br>
Exit Grade: {trade.get("ExitGrade", "")}<br>
Bot Grade: {trade.get("BotGrade", "")}<br>
Reason: {trade.get("GradeReason", "")}<br>
...
"""
    ...
    html += f"""
<div class="card">
<h2>Last Bot Trade Review</h2>
Entry Reason: {last_review.get("entry_reason", "N/A")}<br>
Exit Reason: {last_review.get("exit_reason", "N/A")}<br>
Entry Grade: {last_review.get("entry_grade", "N/A")}<br>
Exit Grade: {last_review.get("exit_grade", "N/A")}<br>
Final P/L: {fmt_money(last_review.get("final_pl"))}<br>
Max Profit During Trade: {fmt_money(last_review.get("max_profit"))}<br>
Max Drawdown: {fmt_money(last_review.get("max_drawdown"))}<br>
Hold Time: {last_review.get("hold_time", "N/A")}<br>
Was this Bot or Human? {last_review.get("human_or_bot", "N/A")}<br>
</div>

<div class="card">
<h2>Trade Analysis</h2>
Bot Win Rate: {trade_analysis["bot"]["win_rate"]:.2f}%<br>
Human Win Rate: {trade_analysis["human"]["win_rate"]:.2f}%<br>
Average Grade: {trade_analysis["all"]["average_grade"]}<br>
Average P/L: {fmt_money(trade_analysis["all"]["average_pl"])}<br>
Best Trade: {fmt_money(trade_analysis["all"]["best_trade"])}<br>
Worst Trade: {fmt_money(trade_analysis["all"]["worst_trade"])}<br>
Average Hold Time: {trade_analysis["all"]["average_hold_time"]}<br>
Total Trades: {trade_analysis["all"]["total_trades"]}<br>
</div>

<div class="card">
<h2>Bot vs Human</h2>
<table style="width:100%; color:white;">
<tr><th align="left">Metric</th><th align="left">Bot</th><th align="left">Human</th></tr>
<tr><td>Win Rate</td><td>{trade_analysis["bot"]["win_rate"]:.2f}%</td><td>{trade_analysis["human"]["win_rate"]:.2f}%</td></tr>
<tr><td>Average P/L</td><td>{fmt_money(trade_analysis["bot"]["average_pl"])}</td><td>{fmt_money(trade_analysis["human"]["average_pl"])}</td></tr>
<tr><td>Average Grade</td><td>{trade_analysis["bot"]["average_grade"]}</td><td>{trade_analysis["human"]["average_grade"]}</td></tr>
<tr><td>Best Trade</td><td>{fmt_money(trade_analysis["bot"]["best_trade"])}</td><td>{fmt_money(trade_analysis["human"]["best_trade"])}</td></tr>
<tr><td>Worst Trade</td><td>{fmt_money(trade_analysis["bot"]["worst_trade"])}</td><td>{fmt_money(trade_analysis["human"]["worst_trade"])}</td></tr>
</table>
</div>
"""
...
Cost Basis: ${{fmtMoney(pl.cost_basis)}}<br>
Current Value: ${{fmtMoney(pl.current_value)}}<br>
<span style="color:${{pnlColor}}; font-weight:bold;">
P/L: ${{fmtMoney(pl.pnl)}}<br>
P/L %: ${{Number(pl.pnl_percent || 0).toFixed(2)}}%
</span><br>
Entry Grade: ${{pl.entry_grade || "N/A"}}<br>
Live Grade: ${{pl.live_grade || "N/A"}}<br>
Current Grade Reason:<br>
${{pl.live_reason || "N/A"}}<br>
Status: OPEN
</div>
<br>`;
    }}).join("");
