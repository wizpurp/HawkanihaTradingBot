...
def submit_option_order(option_symbol, qty, action="buy_to_open"):
    ...
    data = {
        ...
        "type": "market",
        "duration": "day",
        "tag": f"MANUAL_{action}_{option_symbol}"
    }
...
def manual_buy_call():
    ...
    if contract:
        submit_option_order(contract["symbol"], config["contracts"], "buy_to_open")
    return redirect("/")
...
def save_settings():
    ...
    e["cooldown_minutes"] = int(request.form.get("cooldown_minutes", 5))
    e["max_trades_per_day"] = int(request.form.get("max_trades_per_day", 10))
...
"""
    ...
    if positions:
        for pos in positions:
            html += f"""
Symbol: {pos.get("symbol")}<br>
Quantity: {pos.get("quantity")}<br>
Cost Basis: {fmt_money(pos.get("cost_basis"))}<br>
Status: OPEN<br><br>
"""
    else:
        html += "No Position"
